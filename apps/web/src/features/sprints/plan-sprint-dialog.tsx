...
import { useSprintItems } from './use-sprints';

export function PlanSprintDialog({ sprintId }: { sprintId: string }) {
  const [open, setOpen] = useState(false);
  const [selected, setSelected] = useState<string[]>([]);
  const { data } = useWorkItems({ pageSize: 100, sort: 'createdAt', order: 'desc' });
  const { add } = useSprintItems(sprintId, () => setOpen(false));

  useEffect(() => {
    if (open) setSelected([]);
  }, [open]);

  // Only items not already in a sprint are candidates for planning in.
  const candidates = (data?.data ?? []).filter((item) => !item.sprintId);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" size="sm">
          <ListPlus className="h-4 w-4" /> Plan items
        </Button>
      </DialogTrigger>
